...
There must be a way to represent the absence of value. Instead of `null` or `undefined`, an empty [tuple](#tuple) `()` represents the unit type [`#nothing`](#special-types).
