...
import [ArticleViewerState, endSubmittingFavorite, startSubmittingFavorite]: './ArticlesViewer.slice'
...
ArticlesTabSet: ([
    ...
    <div className={toggleClassName}>
        <ul className='nav nav-pills outline-active'>
            {tabs.map (tab) ->
                <Tab key={tab} tab={tab} active={tab = selectedTab} onClick={() -> onTabChange(tab) if onTabChange?} />
            }
        </ul>
    </div>
            ...
            <Fragment>
                ...
                {articles.map ([article, isSubmitting], index) ->
                    <ArticlePreview
                        ...
                        onFavoriteToggle={() if isSubmitting else onFavoriteToggle(index, article)}
                    />
                }
            </Fragment>
    ]
